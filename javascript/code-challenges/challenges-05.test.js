...
const addPurchases = (arr) => {
  // Solution code here...
  const Price = arr.reduce((acc,val)=> acc+val.purchasePrice,0)
  return Price;
};
...
const returnNames = (arr) => {
  ...
  const ArrOfObjNames =[];
  arr.reduce((acc, val) => {
    ArrOfObjNames.push(val.name)
})
ArrOfObjNames.unshift(starWarsData[0].name);
return ArrOfObjNames;
